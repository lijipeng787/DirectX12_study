...
bool ShaderLoader::CreateVSAndPSFromFile(WCHAR *vs_filename,
                                         WCHAR *ps_filename) {
...
bool ShaderLoader::CreateVSFromFile(WCHAR *vs_filename) {
  ...
  if (FAILED(D3DCompileFromFile(vs_filename, nullptr, nullptr, vs_entry_point_,
                                vs_target_version_, compile_flags, 0,
                                &vertex_shader_blob, &last_compile_error_))) {
    if (nullptr != last_compile_error_) {
      char err[256] = {};
      memcpy(err, last_compile_error_.Get()->GetBufferPointer(),
             last_compile_error_.Get()->GetBufferSize());
      assert(0);
    }
    return false;
  }
...
bool ShaderLoader::CreatePSFromFile(WCHAR *ps_filename) {
  ...
  if (FAILED(D3DCompileFromFile(ps_filename, nullptr, nullptr, ps_entry_point_,
                                ps_target_version_, compile_flags, 0,
                                &pixel_shader_blob, &last_compile_error_))) {
    if (nullptr != last_compile_error_) {
      char err[256] = {};
      memcpy(err, last_compile_error_.Get()->GetBufferPointer(),
             last_compile_error_.Get()->GetBufferSize());
