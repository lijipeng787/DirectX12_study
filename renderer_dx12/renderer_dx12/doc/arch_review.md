...
- **光照模型统一 vs 分离**：应该为所有材质提供统一的光照接口，还是允许不同材质使用完全不同的光照结构？建议：统一光源数据存储，但允许材质按需提取与转换，平衡灵活性与维护性。
- **多光源架构设计**：当前单光源架构如何扩展到多光源？是通过数组传递到常量缓冲，还是引入光照剔除与分簇渲染？建议：先实现简单的固定数量光源数组（如最多4个），验证需求后再优化。
