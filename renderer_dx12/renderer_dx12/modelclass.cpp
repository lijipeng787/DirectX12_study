...
ModelMaterial *Model::GetMaterial() { return &material_; }
...
bool Model::InitializeBuffers() {
  ...
  for (UINT i = 0; i < vertex_count_; ++i) {
    vertices[i].position_ =
        DirectX::XMFLOAT3(tem_model_[i].x_, tem_model_[i].y_, tem_model_[i].z_);
    vertices[i].texture_position_ =
        DirectX::XMFLOAT2(tem_model_[i].tu_, tem_model_[i].tv_);
    vertices[i].normal_ = DirectX::XMFLOAT3(
        tem_model_[i].nx_, tem_model_[i].ny_, tem_model_[i].nz_);
    indices[i] = i;
  }
...
bool ModelMaterial::InitializeRootSignature() {
  ...
  // Light b0 pixel
  root_parameters[2].InitAsConstantBufferView(0, 0,
                                              D3D12_SHADER_VISIBILITY_PIXEL);
  // Fog b1 vertex
  root_parameters[3].InitAsConstantBufferView(1, 0,
                                              D3D12_SHADER_VISIBILITY_VERTEX);
...
bool ModelMaterial::UpdateMatrixConstant(const XMMATRIX &world,
                                         const XMMATRIX &view,
...
bool ModelMaterial::UpdateLightConstant(const XMFLOAT4 &ambient_color,
