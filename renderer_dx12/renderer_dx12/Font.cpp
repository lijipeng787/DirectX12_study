...
void Font::BuildVertexArray(void* vertices, WCHAR *sentence, float drawX, float drawY) {
	...
	for (unsigned int i = 0; i < numLetters; ++i) {
		...
		if (0 == letter) {
			...
			vertexPtr[index].texture_position_ = DirectX::XMFLOAT2(font_[letter].left_, 0.0f);
			++index;

			// Bottom right.
			vertexPtr[index].position_ = DirectX::XMFLOAT3((drawX + font_[letter].size_), (drawY - 16), 0.0f);
			vertexPtr[index].texture_position_ = DirectX::XMFLOAT2(font_[letter].right_, 1.0f);
			++index;

			// Bottom left.
			vertexPtr[index].position_ = DirectX::XMFLOAT3(drawX, (drawY - 16), 0.0f);
			vertexPtr[index].texture_position_ = DirectX::XMFLOAT2(font_[letter].left_, 1.0f);
			++index;
			...
			vertexPtr[index].texture_position_ = DirectX::XMFLOAT2(font_[letter].right_, 0.0f);
			++index;
