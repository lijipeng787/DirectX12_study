...

	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr)) {
				...
				upload_resource_desc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
				upload_resource_desc.Alignment = 0;
				upload_resource_desc.Width = width;
				upload_resource_desc.Height = height;
				upload_resource_desc.DepthOrArraySize = 1;
				upload_resource_desc.MipLevels = static_cast<UINT16>(mipCount - skipMip);
				upload_resource_desc.Format = format;
				upload_resource_desc.SampleDesc.Count = 1;
				upload_resource_desc.SampleDesc.Quality = 0;
				upload_resource_desc.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN;
				upload_resource_desc.Flags = D3D12_RESOURCE_FLAG_NONE;

				auto upload_data_size = GetRequiredIntermediateSize(*texture, 0, subresourceCount);

				ID3D12Resource *upload_resource = nullptr;
				if (FAILED(d3dDevice->CreateCommittedResource(
					&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
					D3D12_HEAP_FLAG_NONE,
					&CD3DX12_RESOURCE_DESC::Buffer(upload_data_size),
				...
				if (FAILED(d3dDevice->CreateCommandQueue(&queue_desc, IID_PPV_ARGS(&command_queue)))) {
					return false;
				}

				ID3D12CommandAllocator *command_allocator = nullptr;
				if (FAILED(d3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&command_allocator)))) {
					return E_UNEXPECTED;
				}
				...
				UpdateSubresources(command_list, *texture, upload_resource, 0, 0, subresourceCount, initData.get());
				command_list->ResourceBarrier(
					1,
					&CD3DX12_RESOURCE_BARRIER::Transition(
						*texture,
