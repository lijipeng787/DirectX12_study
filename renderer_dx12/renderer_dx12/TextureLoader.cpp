...
namespace ResourceLoader {
	...
	bool TextureLoader::LoadTexturesByNameArray(unsigned int num_textures, WCHAR **texture_filename_arr) {
		...
		for (unsigned int index = 0; index < num_textures; ++index) {
			...
			if (FAILED(CreateDDSTextureFromFile(device.Get(), texture_filename_arr[index], 0, false, &tem_texture, handle))) {
				return false;
			}
