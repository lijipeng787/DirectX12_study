...
auto ReflectionFloorMaterial::Initialize() -> bool {
  ...
  if (!reflection_constant_buffer_.Initialize(device_)) {
    return false;
  }
...
auto ReflectionFloorMaterial::GetReflectionConstantBuffer() const
    -> ResourceSharedPtr {
  return reflection_constant_buffer_.GetResource();
}
...
auto ReflectionFloorMaterial::UpdateReflectionConstant(
  ...
  return reflection_constant_buffer_.Update(reflection_constant_data_);
}
