...
void Position::MoveForward(bool key_down) {
  if (key_down) {
    forward_speed_ = (forward_speed_ < frame_time_ * 1.0f)
...
void Position::MoveBackward(bool key_down) {
  if (key_down) {
    backward_speed_ = (backward_speed_ < frame_time_ * 1.0f)
...
void Position::MoveUpward(bool key_down) {
  if (key_down) {
    upward_speed_ = (upward_speed_ < frame_time_ * 1.0f)
                        ? (upward_speed_ += frame_time_ * 0.001f)
                        : frame_time_ * 1.0f;
  } else {
    upward_speed_ = 0.0f;
  }
...
void Position::MoveDownward(bool key_down) {
  if (key_down) {
    downward_speed_ = (downward_speed_ < frame_time_ * 1.0f)
                          ? (downward_speed_ += frame_time_ * 0.001f)
                          : frame_time_ * 1.0f;
  } else {
...
void Position::TurnLeft(bool key_down) {
  if (key_down) {
    turn_left_speed_ = (turn_left_speed_ < frame_time_ * 1.0f)
                           ? turn_left_speed_ += frame_time_ * 0.001f
                           : frame_time_ * 1.0f;
  } else {
...
void Position::TurnRight(bool key_down) {
  if (key_down) {
    turn_right_speed_ = (turn_right_speed_ < frame_time_ * 1.0f)
                            ? turn_right_speed_ += frame_time_ * 0.001f
                            : frame_time_ * 1.0f;
  } else {
...
void Position::LookUpward(bool key_down) {
  if (key_down) {
    look_up_speed_ = (look_up_speed_ < frame_time_ * 75.0f)
                         ? look_up_speed_ += frame_time_ * 0.001f
                         : frame_time_ * 75.0f;
  } else {
    look_up_speed_ = 0.0f;
  }
...
void Position::LookDownward(bool key_down) {
  if (key_down) {
    look_down_speed_ = (look_down_speed_ < frame_time_ * 75.0f)
                           ? look_down_speed_ += frame_time_ * 0.001f
                           : frame_time_ * 75.0f;
  } else {
